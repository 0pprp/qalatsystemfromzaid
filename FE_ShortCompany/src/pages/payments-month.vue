...
// بيانات "شهر التسديدات" وقائمة المندوبين
const monthPaymentsData = ref([])
...
const headers = [
  ...
  { title: 'سعر البيع', key: 'amountTotalSalesDenar' },
...
// إضافة أعمدة amount1 إلى amount30 (أسماء الحقول بأول حرف صغير)
for (let i = 1; i <= 30; i++) {
...
const totals = computed(() => [
  {
    ...
    value: monthPaymentsData.value.length,
    title: 'عدد العملاء',
    color: "primary",
  ...
  },
  {
    ...
    value: formattedNumber(
      monthPaymentsData.value.reduce((sum, row) => sum + (row.amountTotalSalesDenar || 0), 0),
    ),
    title: 'سعر البيع',
    color: "success",
  ...
  },
  {
    ...
    value: formattedNumber(
      monthPaymentsData.value.reduce((sum, row) => sum + (row.costTotalSales || 0), 0),
    ),
    title: 'سعر الشراء',
    color: "warning",
  ...
  },
  {
    ...
    value: formattedNumber(
      monthPaymentsData.value.reduce((sum, row) => sum + (row.amountDaySales || 0), 0),
    ),
    title: 'القسط',
    color: "info",
  ...
  },
  {
    ...
    value: formattedNumber(
      monthPaymentsData.value.reduce((sum, row) => sum + (row.receiptsTotal || 0), 0),
    ),
    title: 'الواصل',
    color: "success",
    gradient: "linear-gradient(135deg, #8E2DE2 0%, #4A00E0 100%)",
  },
  {
    ...
    value: formattedNumber(
      monthPaymentsData.value.reduce((sum, row) => sum + (row.amountRemaining || 0), 0),
    ),
    title: 'الباقي',
    color: "error",
...
// دالة جلب بيانات "شهر التسديدات" باستخدام الفلاتر (من خلال API Customers_GetMonthReceipt)
async function fetchMonthPayments() {
  try {
    ...
    const response = await axios.get(`${apiUrl}Customers/Customers_GetMonthReceipt/${delegateID}&&${showType}`, { headers: authHeader })

    monthPaymentsData.value = response.data
  } catch (error) {
    console.error(error)
  } finally {
    loading.value = false
...
function exportToExcel() {
  const dataToExport = monthPaymentsData.value.map(row => {
    const exportRow = {
      ...
      'تاريخ البيع': row.dateSale ? formattedDate(row.dateSale) : 'لا يوجد',
      'عدد الأيام': row.numberOfDayDevice || 'لا يوجد',
      'سعر البيع': row.amountTotalSalesDenar || 'لا يوجد',
    ...
    for (let i = 1; i <= 30; i++) {
      exportRow[formattedDate(new Date(new Date() - i * 24 * 60 * 60 * 1000))] = row[`amount${i}`] !== undefined ? row[`amount${i}`] : 'لا يوجد'
    }
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "MonthPayments")
  XLSX.writeFile(workbook, "MonthPayments.xlsx")
}
    ...
    <VForm>
      ...
      <VRow>
        ...
        >
          ...
          <template v-slot:[colSlots.amountTotalSalesDenar]="{ item }">
