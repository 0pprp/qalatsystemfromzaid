...
// فلترة الخزائن محلياً حسب اسم الخزينة وترتيبها تصاعدياً حسب boxID
const filteredBoxes = computed(() => {
  let boxes = boxData.value
  
  // تطبيق الفلترة إذا كان هناك نص بحث
  if (searchText.value) {
  ...
  // ترتيب الخزائن تصاعدياً حسب boxID
  return boxes.slice().sort((a, b) => a.boxID - b.boxID)
})
