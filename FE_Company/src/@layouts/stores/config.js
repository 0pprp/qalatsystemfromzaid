...
export const cookieRef = (key, defaultValue) => {
  return useCookie(namespaceConfig(key), { default: () => defaultValue, maxAge: 60 * 60 * 24 * 365 })
}
export const useLayoutConfigStore = defineStore('layoutConfig', () => {
  const route = useRoute()

  // 👉 Navbar Type
  const navbarType = cookieRef('navbarType_v2', layoutConfig.navbar.type)

  // 👉 Navbar Type
  const isNavbarBlurEnabled = cookieRef('isNavbarBlurEnabled_v2', layoutConfig.navbar.navbarBlur)

  // 👉 Vertical Nav Collapsed
  const isVerticalNavCollapsed = cookieRef('isVerticalNavCollapsed_v2', layoutConfig.verticalNav.isVerticalNavCollapsed)

  // 👉 App Content Width
  const appContentWidth = cookieRef('appContentWidth_v2', layoutConfig.app.contentWidth)

  // 👉 App Content Layout Nav
  const appContentLayoutNav = cookieRef('appContentLayoutNav_v2', layoutConfig.app.contentLayoutNav)
  ...
  // 👉 Horizontal Nav Type
  const horizontalNavType = cookieRef('horizontalNavType_v2', layoutConfig.horizontalNav.type)

  //  👉 Horizontal Nav Popover Offset
  const horizontalNavPopoverOffset = ref(layoutConfig.horizontalNav.popoverOffset)

  // 👉 Footer Type
  const footerType = cookieRef('footerType_v2', layoutConfig.footer.type)
  ...
  // 👉 RTL
  const isAppRTL = cookieRef('isAppRTL_v2', layoutConfig.app.isRTL)
