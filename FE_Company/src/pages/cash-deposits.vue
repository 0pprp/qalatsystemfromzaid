...
// بيانات إضافات الخزائن
const addToBoxData = ref([])
...
// فلاتر البحث: البحث من التاريخ إلى التاريخ واختيار الخزينة
const filters = ref({
...
// تعريف أعمدة جدول الإضافات (أسماء الحقول تبدأ بحرف صغير)
const headers = [
...
const totals = computed(() => [
  {
    ...
    value: addToBoxData.value.length,
    title: 'عدد الإضافات الكلي',
    color: "primary",
  ...
  },
  {
    ...
    value: formattedNumber(
      addToBoxData.value.reduce((sum, item) => sum + (item.amountDenar || 0), 0),
    ),
    title: 'المبلغ الكلي',
    color: "success",
...
// دالة جلب بيانات إضافات الخزائن من API باستخدام الفلاتر
async function fetchAddToBoxData() {
  try {
    loading.value = true
    ...
    // استدعاء API الخاص بالإضافات: AddToBoxs_GetByDateByboxID/{fromDate}&&{toDate}&&{boxID}
    const response = await axios.get(`${apiUrl}Accounts/AddToBoxs_GetByDateByboxID/${fromDate}&&${toDate}&&${boxID}`, { headers: authHeader })

    addToBoxData.value = response.data
  } catch (error) {
    console.error(error)
  } finally {
    loading.value = false
...
// دالة تصدير بيانات إضافات الخزائن إلى Excel
function exportToExcel() {
  const dataToExport = addToBoxData.value.map(item => ({
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "AddToBox")
  XLSX.writeFile(workbook, "AddToBox.xlsx")
}
...
async function deleteAccount() {
  if (accountID.value) {
    try {
      ...
      await axios.delete(`${apiUrl}Accounts/AddToBox_Delete/${accountID.value}`, { headers: authHeader })

      const index = addToBoxData.value.findIndex(sale => sale.addToBoxID === accountID.value)
      if (index !== -1) {
        addToBoxData.value.splice(index, 1)
      }
      confirmAccountDialog.value = false
    } catch (error) {
      console.error(error)
          ...
          <template #item.delete="{ item }">
            <div class="d-flex gap-1">
              <VBtn
                ...
                @click="openDeleteDialog(item.addToBoxID)"
              >
                حذف
              </VBtn>
            </div>
