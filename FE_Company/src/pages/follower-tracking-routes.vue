...
import { ftGetFollowers, ftGetRoute } from '@/composables/followerTrackingApi'
...
onMounted(async () => {
  ...
  await ensureMap()
  try {
    followers.value = await ftGetFollowers()
  }
  catch {
...
async function loadLocations() {
  if (!followerId.value) {
    toast.warning('اختر المتابع أولاً')
  ...
  try {
    const payload = await ftGetRoute(followerId.value, date.value)
    ...
    shiftSummary.value = {
      startedAt: formatIraqTime(shift?.startedAtUtc ?? shift?.StartedAtUtc),
      endedAt: formatIraqTime(shift?.closedAtUtc ?? shift?.ClosedAtUtc)
      ...
      <VRow dense>
        ...
        >
          <VSelect
            v-model="followerId"
            :items="followers"
            item-title="followerName"
            item-value="followerId"
            label="المتابع"
            hide-details
          />
        </VCol>
