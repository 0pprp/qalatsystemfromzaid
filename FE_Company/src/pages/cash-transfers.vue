...
// بيانات النقل بين الخزائن
const transferBoxsData = ref([])
...
// تعريف أعمدة جدول النقل بين الخزائن (أسماء الحقول تبدأ بحرف صغير)
const headers = [
  { title: 'المبلغ', key: 'amountDenar' },
  { title: 'التاريخ', key: 'dateCreate' },
  { title: 'من الخزينة', key: 'fromBoxName' },
  { title: 'الى الخزينة', key: 'toBoxName' },
...
const totals = computed(() => [
  {
    ...
    value: transferBoxsData.value.length,
    title: 'عدد النقل الكلي',
    color: "primary",
  ...
  },
  {
    ...
    value: formattedNumber(
      transferBoxsData.value.reduce((sum, item) => sum + (item.amountDenar || 0), 0),
    ),
    title: 'المبلغ الكلي',
    color: "success",
...
// دالة جلب بيانات النقل بين الخزائن من API باستخدام فلاتر التاريخ
async function fetchTransferBoxsData() {
  try {
    loading.value = true
    ...
    // نفترض أن نقطة النهاية هي:
    // TransferBoxs_GetByDate/{fromDate}&&{toDate}
    const response = await axios.get(`${apiUrl}Accounts/TransferBoxs_GetByDate/${fromDate}&&${toDate}`, { headers: authHeader })

    transferBoxsData.value = response.data
  } catch (error) {
    console.error(error)
  } finally {
    loading.value = false
...
// دالة تصدير بيانات النقل بين الخزائن إلى Excel
function exportToExcel() {
  const dataToExport = transferBoxsData.value.map(item => ({
    'المبلغ': item.amountDenar || 'لا يوجد',
    'التاريخ': item.dateCreate ? formattedDate(item.dateCreate) : 'لا يوجد',
    'من الخزينة': item.fromBoxName || 'لا يوجد',
    'الى الخزينة': item.toBoxName || 'لا يوجد',
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "TransferBoxs")
  XLSX.writeFile(workbook, "TransferBoxs.xlsx")
}
...
async function deleteAccount() {
  if (accountID.value) {
    try {
      ...
      await axios.delete(`${apiUrl}Accounts/TransferBoxs_Delete/${accountID.value}`, { headers: authHeader })

      const index = transferBoxsData.value.findIndex(sale => sale.transferBoxID === accountID.value)
      if (index !== -1) {
        transferBoxsData.value.splice(index, 1)
      }
      confirmAccountDialog.value = false
    } catch (error) {
      console.error(error)
  ...
  <div>
    ...
    <VCard class="pa-10">
      <VForm>
        ...
        <!-- عرض بيانات النقل بين الخزائن -->
        <VRow>
          <VDataTable
            :headers="headers"
            :items="transferBoxsData"
            :items-per-page="50"
          ...
          >
            ...
            <template #item.fromBoxName="{ item }">
              <div>
                {{ item.fromBoxName || 'لا يوجد' }}
              </div>
            </template>
            <template #item.toBoxName="{ item }">
              <div>
                {{ item.toBoxName || 'لا يوجد' }}
              </div>
            </template>
            ...
            <template #item.delete="{ item }">
              <div class="d-flex gap-1">
                <VBtn
                  ...
                  @click="openDeleteDialog(item.transferBoxID)"
                >
                  حذف
                </VBtn>
              </div>
