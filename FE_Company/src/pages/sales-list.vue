...
const headers = [
  ...
  { title: 'تعديل', key: 'updateDiscount' },
...
// دالة تحديد الحالة (منسوخة من customers-list)
function getStatus(item) {
  if (item.isFakeSale === true || item.isFakeSale === 'true') {
    return { text: 'وهمي', color: 'warning' }
  }
  ...
  // ملاحظة: تأكد من أن الحقل isLegal موجود في بيانات المبيعات أو سيتم تخطيه
...
async function sendMessage(item) {
  try {
    ...
    const response = await axios.get(
      `${apiUrl}CustomersSales/CustomersSales_GetMessage/${item.customerSaleID}`, { headers: authHeader },
    )

    // Handle both new MessageDTO ({message, portalUrl}) and legacy string response
    const message = response.data?.message || (typeof response.data === 'string' ? response.data : null)

    if (!message) {
      toast.error('لم يتم العثور على نص الرسالة')
      
      return
    }

    const phoneNumber = convertToInternationalPhoneNumber(item.phoneNumber)

    if (!phoneNumber) {
      toast.error('رقم الهاتف غير صحيح أو غير موجود')
    ...
    toast.error(typeof errorMsg === 'string' ? errorMsg : 'حدث خطأ أثناء إرسال الرسالة')
  }
}


function convertToInternationalPhoneNumber(localPhoneNumber) {
  if (!localPhoneNumber || typeof localPhoneNumber !== 'string') {
    return null
  }
  let cleanedPhoneNumber = localPhoneNumber.replace(/\D/g, '')
  if (cleanedPhoneNumber.startsWith('0')) {
    cleanedPhoneNumber = '+964' + cleanedPhoneNumber.slice(1)
  } else {
    cleanedPhoneNumber = '+964' + cleanedPhoneNumber
  }
    ...
    <VForm>
      ...
      <VRow>
        ...
        >
          ...
          <template #item.boundNumber="{ item }">
            <div>
          ...
          <template #item.numberOfItemsSales="{ item }">
            <div>
          ...
          <template #item.updateDiscount="{ item }">
