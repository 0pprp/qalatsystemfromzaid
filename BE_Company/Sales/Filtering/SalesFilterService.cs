...
namespace BE_Company.Sales.Filtering
{
    ...
    public sealed class SalesFilterService : ISalesFilterService
    {
        ...
        {
            EnsureFilterEmployee(actor);
            var row = await RequireInScope(actor.EmployeeId, id, ct);
            return row;
        }
            ...
            CancellationToken ct)
        {
            ...
            var (ok, current) = await _repo.TryTransitionAsync(
                id, row.FilterStatus, newStatus, actor.EmployeeId, note, reason, ct);
            if (!ok)
            {
                if (current == null)
                {
            ...
            return await RequireInScope(actor.EmployeeId, id, ct);
        }

        private async Task<SalesFilterDetailDTO> RequireInScope(int userId, int id, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(row.CityValue)
                || !await _repo.UserHasCityAsync(userId, row.CityValue, ct))
            {
                // Hide existence of out-of-scope rows.
                throw new SalesCompleteException(StatusCodes.Status404NotFound, "الطلب غير موجود.");
            }
