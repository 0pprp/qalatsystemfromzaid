CREATE proc [dbo].[GetCustomerPaymentByDateDelegate]
@DelegateID int = NULL,
...
DelegateID,
 ...
 (SELECT        TOP 1 DateCreate
                             ...
                             (SELECT        ISNULL(SUM(AmountDenar), 0)
							   ...
							   (select ISNULL(sum(AmountDenar),0) from View_CustomersPaymentsDelegate where CustomerID=Customers.CustomerID and CONVERT(date, PaymentDate)=@PaymentDate)as Amount,
...
where DelegateID=@DelegateID  and (ROUND(ISNULL
                             ...
                             (SELECT        ISNULL(SUM(AmountDenar), 0)
                               FROM            dbo.View_AddToBox
                               WHERE        (CustomerIDPayment = dbo.Customers.CustomerID)), 0), - 3))>0
