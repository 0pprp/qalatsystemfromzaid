CREATE proc [dbo].[GetCustomerPaymentByDateDelegateZero]
@DelegateID int = NULL,
...
DelegateID,
 ...
 (SELECT        TOP 1 DateCreate
                             ...
                             (SELECT        ISNULL(SUM(AmountDenar), 0)
							   ...
							   (select ISNULL(sum(AmountDenar),0) from View_CustomersPaymentsDelegate where CustomerID=Customers.CustomerID and CONVERT(date, PaymentDate)=CONVERT(date, @PaymentDate))as Amount,
...
from Customers
where DelegateID=@DelegateID  and (ROUND(ISNULL
                             ((SELECT        ISNULL(sum(AmountTotalSalesDenar), 0)
                                 FROM            View_CustomersSalesDelegate
                                 WHERE        CustomerID = Customers.CustomerID) -
                             (SELECT        ISNULL(SUM(AmountDenar), 0)
                               FROM            dbo.View_AddToBox
                               WHERE        (CustomerIDPayment = dbo.Customers.CustomerID)), 0), - 3))>0 and (select ISNULL(sum(AmountDenar),0) from View_CustomersPaymentsDelegate where CustomerID=Customers.CustomerID and CONVERT(date, PaymentDate)>=CONVERT(date, GETDATE()-30) and CONVERT(date, PaymentDate)<=CONVERT(date, GETDATE()))>0
