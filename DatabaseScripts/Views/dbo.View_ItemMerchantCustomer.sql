create   view [dbo].[View_ItemMerchantCustomer]
AS
WITH UserData AS (SELECT        UserID, UserName
    ...
    SELECT        dbo.ItemMerchant.ItemMerchantID, dbo.ItemMerchant.MerchantID, dbo.ItemMerchant.UserID, dbo.ItemMerchant.ItemMerchantName, dbo.ItemMerchant.Specifications, dbo.ItemMerchant.Notes, 
                              dbo.ItemMerchant.ItemMerchantState, dbo.ItemMerchant.CategoryID, dbo.ItemMerchant.Price, ROUND(dbo.ItemMerchant.Price * 1.05, - 3) AS AmountCash, ROUND(dbo.ItemMerchant.Price * 1.5 / 100, - 3) AS AmountDay, 
                              ROUND(dbo.ItemMerchant.Price * 1.4 / 18, - 3) AS AmountWeek, ROUND(dbo.ItemMerchant.Price * 1.3 / 5, - 3) AS AmountMonth, UserData_1.UserName, MerchantData_1.MerchantName, CategoryData_1.CategoryName,
                                  (SELECT        TOP (1) Image
