...
class TrackingBootReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        LocationForegroundService.log("FOLLOWER_BOOT_OR_REPLACE action=$action")
        ...
        try {
            ...
            LocationForegroundService.log("FOLLOWER_BOOT_SKIP reason=meta-read error=${e.message}")
            return
        }
        ...
        if (!active || shiftId <= 0 || cutoff <= 0L || now >= cutoff) {
            LocationForegroundService.log(
                "FOLLOWER_BOOT_SKIP reason=no-active-shift shiftId=$shiftId active=$active cutoff=$cutoff",
            )
            return
        }
        ...
        try {
            ...
            LocationForegroundService.log("FOLLOWER_BOOT_RESTART_TRACKING shiftId=$shiftId")
        } catch (e: Exception) {
            LocationForegroundService.log("FOLLOWER_UPLOAD_FAILED status=boot-start error=${e.message}")
        }
    }
}
