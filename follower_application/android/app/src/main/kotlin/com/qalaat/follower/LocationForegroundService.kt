...
class LocationForegroundService : Service() {
    ...
    private val callback = object : LocationCallback() {
        override fun onLocationResult(result: LocationResult) {
            ...
            log(
                "FOLLOWER_LOCATION_RECEIVED lat=${loc.latitude} lng=${loc.longitude} accuracy=${
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        try {
            ...
            log("FOLLOWER_DATABASE_ERROR startForeground ${e.javaClass.simpleName}: ${e.message}")
            stopSelf()
        ...
        val dbOk = try {
            ...
            log("FOLLOWER_DATABASE_ERROR openDb ${e.javaClass.simpleName}: ${e.message}")
            false
        }
        if (!dbOk) {
        ...
        try {
            if (intent?.action == ACTION_STOP || intent?.action == ACTION_FLUSH_STOP) {
                ...
                Thread {
                    ...
                    handler.post {
                        stopForegroundCompat()
                        stopSelf()
                        log("FOLLOWER_SHIFT_ENDED")
                        log("FOLLOWER_TRACKING_SERVICE_STOPPED")
                    }
                }.start()
                return START_NOT_STICKY
            }
            ...
            if (shiftId <= 0 || System.currentTimeMillis() >= cutoffAtUtcMs) {
                log("FOLLOWER_SERVICE_STOPPED reason=no-active-shift shiftId=$shiftId")
                stopCollecting(flush = false)
                return START_NOT_STICKY
            }
            running = true
            log("FOLLOWER_TRACKING_SERVICE_STARTED shiftId=$shiftId officialIntervalMs=$officialIntervalMs")
            log("FOLLOWER_SHIFT_STARTED shiftId=$shiftId startedAtUtcMs=$shiftStartedAtUtcMs cutoffAtUtcMs=$cutoffAtUtcMs")
            startUpdates()
            seedLastLocation()
            handler.removeCallbacks(cutoffStop)
            handler.removeCallbacks(officialTick)
            ...
            log("FOLLOWER_DATABASE_ERROR onStartCommand ${e.javaClass.simpleName}: ${e.message}")
            running = false
            handler.removeCallbacks(cutoffStop)
            handler.removeCallbacks(officialTick)
            handler.removeCallbacks(syncFlush)
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        log("FOLLOWER_TASK_REMOVED keep running")
    }
    ...
    private fun startUpdates() {
        ...
        try {
            ...
            log("FOLLOWER_LIVE_UPLOAD_FAILED status=permission error=${e.message}")
            stopSelf()
        }
    }
    ...
    private fun persistDueOfficialPoints() {
        ...
        try {
            ...
            val start = if (shiftStartedAtUtcMs > 0L) shiftStartedAtUtcMs else {
                log("FOLLOWER_LOCATION_RETRY reason=missing-shift-start")
                return
            }
            ...
            if (loc == null || !isUsable(loc)) {
                log("FOLLOWER_LOCATION_RETRY reason=no-fix slotDueAtMs=$due")
                return
            }
            ...
            if (insertOfficialPoint(loc, due, index)) {
                ...
                log("FOLLOWER_LOCATION_CAPTURED seq=$index slotDueAtUtcMs=$due lat=${loc.latitude} lng=${loc.longitude}")
                Thread {
                    flushPendingToServer()
                }.start()
            }
        } catch (e: Exception) {
            log("FOLLOWER_DATABASE_ERROR persistDue ${e.javaClass.simpleName}: ${e.message}")
        }
    }
    ...
    private fun scheduleNextOfficialTick() {
        ...
        log("FOLLOWER_NEXT_SLOT_DUE delayMs=$delay dueAtUtcMs=$next intervalMs=$officialIntervalMs")
    ...
    private fun insertOfficialPoint(loc: Location, slotUtcMs: Long, sequenceIndex: Long): Boolean {
        ...
        if (id > 0L) {
            log("FOLLOWER_LOCATION_SENT localId=$id seq=$seq slotDueAtUtc=$captured actualCapturedAtUtc=$actual")
            return true
        }
        return false
    }
    ...
    private fun uploadLiveLocation(loc: Location?) {
        ...
        try {
            ...
            if (base.isEmpty() || auth.isEmpty() || sid <= 0) {
                log("FOLLOWER_LIVE_UPLOAD_FAILED status=missing-config")
                return
            }
            val body = JSONObject()
            ...
            try {
                ...
                if (code in 200..299) {
                    log("FOLLOWER_LIVE_UPLOAD_SUCCESS status=$code")
                } else {
                    log("FOLLOWER_LIVE_UPLOAD_FAILED status=$code error=${text.take(240)}")
                }
            } finally {
                conn.disconnect()
            }
        } catch (e: Exception) {
            log("FOLLOWER_LIVE_UPLOAD_FAILED status=exception error=${e.javaClass.simpleName}:${e.message}")
        } finally {
    ...
    private fun writeMeta(key: String, value: String) {
        try {
            ...
            log("FOLLOWER_DATABASE_ERROR writeMeta key=$key ${e.javaClass.simpleName}: ${e.message}")
        }
    }
    ...
    private fun readMeta(key: String): String? {
        return try {
            ...
            log("FOLLOWER_DATABASE_ERROR readMeta key=$key ${e.javaClass.simpleName}: ${e.message}")
            null
        }
    }
    ...
    private fun flushPendingToServer() {
        ...
        try {
            ...
            if (base.isEmpty() || auth.isEmpty() || sid <= 0 || db == null) {
                log("FOLLOWER_ROUTE_UPLOAD_FAILED status=missing-config baseEmpty=${base.isEmpty()} tokenEmpty=${auth.isEmpty()} shiftId=$sid")
                return
            }
            ...
            try {
                ...
                if (code in 200..299) {
                    ...
                    if (keepPending) {
                        ...
                        log("FOLLOWER_ROUTE_UPLOAD_FAILED status=$code error=all-rejected rejected=${parsed.rejected}")
                    } else {
                        markStatus(sid, seqs, "Synced")
                        log("FOLLOWER_LOCATION_ACCEPTED accepted=${parsed?.accepted ?: "?"} duplicates=${parsed?.duplicates ?: "?"} rejected=${parsed?.rejected ?: "?"}")
                        log("FOLLOWER_ROUTE_UPLOAD_SUCCESS accepted=${parsed?.accepted ?: "?"} duplicates=${parsed?.duplicates ?: "?"} rejected=${parsed?.rejected ?: "?"}")
                    }
                } else {
                    markStatus(sid, seqs, "Failed", incrementRetry = true)
                    log("FOLLOWER_LOCATION_RETRY status=$code error=${text.take(240)}")
                    log("FOLLOWER_ROUTE_UPLOAD_FAILED status=$code error=${text.take(240)}")
                }
            } finally {
                conn.disconnect()
            }
        } catch (e: Exception) {
            log("FOLLOWER_ROUTE_UPLOAD_FAILED status=exception error=${e.javaClass.simpleName}:${e.message}")
        } finally {
            flushing.set(false)
            releaseWakeLock()
        }
    ...
    private fun followerUrl(base: String, path: String, asyncId: String): java.net.URL {
        val encoded = URLEncoder.encode(asyncId, "UTF-8")
        return java.net.URL(base.trimEnd('/') + path + "?asyncId=" + encoded)
    }
    ...
    private fun acquireWakeLock() {
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "FollowerTracking:flush").also {
    ...
    companion object {
        const val TAG = "FOLLOWER_Tracking"
        const val DB_NAME = "follower_tracking.db"
        const val ACTION_STOP = "com.qalaat.follower.STOP_TRACKING"
        const val ACTION_FLUSH_STOP = "com.qalaat.follower.FLUSH_STOP_TRACKING"
        ...
        const val CHANNEL_ID = "follower_shift"
