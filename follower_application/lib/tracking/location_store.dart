...
class MemoryLocationStore implements LocationPointStore {
  ...
  Future<void> insert(LocalLocationPoint point) async {
    if (points.any((p) =>
        p.shiftId == point.shiftId &&
        p.deviceSequence == point.deviceSequence)) {
      return;
    }
    points.add(point);
  }
  ...
  Future<List<LocalLocationPoint>> pending({int limit = 200}) async {
    return points
        .where((p) => p.syncStatus == 'Pending' || p.syncStatus == 'Failed')
        .take(limit)
        .toList();
  }
  ...
  Future<void> markStatus(
      List<int> sequences, int shiftId, String status) async {
    for (final p in points.where((p) =>
        p.shiftId == shiftId && sequences.contains(p.deviceSequence))) {
      p.syncStatus = status;
    }
  }
