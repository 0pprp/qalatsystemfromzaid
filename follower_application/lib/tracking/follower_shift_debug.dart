...
class FollowerShiftDebug {
  ...
  static void log(String message) {
    ...
    debugPrint('FOLLOWER_SHIFT: ${sanitize(message)}');
  }

  static void logError(String stage, Object error, [StackTrace? stack]) {
    if (!kDebugMode) return;
    debugPrint('FOLLOWER_SHIFT ERROR at $stage: ${error.runtimeType}');
    debugPrint('FOLLOWER_SHIFT ERROR message: ${sanitize(error.toString())}');
    if (stack != null) {
      debugPrint('FOLLOWER_SHIFT ERROR stack:\n$stack');
    }
  }
  ...
  static String sanitize(String text) {
    ...
    value = value.replaceAll(
      RegExp(r'(asyncId|password|token|secret)\s*[:=]\s*\S+', caseSensitive: false),
      '[redacted]',
    );
    return value;
  }
  ...
  static String apiFailure(Object error) {
    ...
    if (error is FollowerApiException) {
