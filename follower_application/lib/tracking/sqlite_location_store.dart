...
''';
  ...
  Future<void> init() async {
    ...
    _db = await openDatabase(
      p.join(dir, 'follower_tracking.db'),
      version: 1,
      singleInstance: true,
  ...
  Future<int> nextSequence(int shiftId) async {
    await init();
    final rows =
        await db.query('tracking_meta', where: 'key = ?', whereArgs: ['seq_$shiftId']);
    final current =
        rows.isEmpty ? 0 : int.tryParse('${rows.first['value']}') ?? 0;
    final next = current + 1;
    await db.insert(
      'tracking_meta',
      {'key': 'seq_$shiftId', 'value': '$next'},
  ...
  Future<int> pendingCount() async {
    ...
    final row = await db.rawQuery(
        "SELECT COUNT(*) AS c FROM local_location_points WHERE sync_status != 'Synced'");
