...
class PaymentsWeekScreen extends StatefulWidget {
  const PaymentsWeekScreen({super.key});

  @override
  State<PaymentsWeekScreen> createState() => _PaymentsWeekScreenState();
}

class _PaymentsWeekScreenState extends State<PaymentsWeekScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      // Use the correct endpoint based on FE_ShortCompany structure
      final response = await _api.get('Customers/Customers_GetWeekReceipt/$delegate&&$_showType');
      if (response.data is List) {
        setState(() {
          _payments = (response.data as List).map((j) => WeekPaymentModel.fromJson(j, days: 7)).toList();
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('تسديدات الأسبوع')),
      body: _isLoading
                  ...
                  if (_payments.isEmpty)
                    ...
                    ..._payments.map((p) => Card(
                          ...
                          child: ExpansionTile(
                            ...
                            trailing: Text(Formatters.formatCurrency(p.amountRemaining), style: TextStyle(color: Colors.red[700])),
                            children: [
                              Padding(
                                padding: const EdgeInsets.all(12),
                                child: SingleChildScrollView(
                                  scrollDirection: Axis.horizontal,
                                  child: DataTable(
                                    columns: List.generate(7, (i) => DataColumn(label: Text('يوم ${i + 1}', style: const TextStyle(fontSize: 10)))),
                                    rows: [
                                      DataRow(cells: List.generate(7, (i) => DataCell(Text(Formatters.formatCurrency(p.amounts.length > i ? p.amounts[i] : 0), style: const TextStyle(fontSize: 10))))),
                                    ],
                                  ),
                                ),
                              ),
